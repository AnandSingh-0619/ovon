...
#SBATCH --output=slurm_logs/train/ovon-ver-%j.out
#SBATCH --error=slurm_logs/train/ovon-ver-%j.err
#SBATCH --gpus a40:4
#SBATCH --nodes 1
...
TENSORBOARD_DIR="tb/objectnav/train/vertf_tf_${JOB_ID}"
CHECKPOINT_DIR="data/new_checkpoints/objectnav/vertf2/"
LOG_DIR="Logs/train_ver_tf_RL_${JOB_ID}.log"

srun python -um ovon.run \
  --run-type train \
  --exp-config config/experiments/transformer_rl.yaml \
  habitat_baselines.trainer_name="ver_transformer" \
